...
test.describe('Admin Projects Page', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    await sharedPage.waitForSelector('[data-testid="admin-data-grid"], .MuiCircularProgress-root, tr, .bg-white.border-b.border-gray-100', { timeout: 15000 });
  });
  ...
  test.describe('Data Loading & Display', () => {
    ...
    test('should load projects page without errors', async () => {
      ...
      const dataElements = page.locator('tr:not(:first-child), .bg-white.border-b.border-gray-100');
    ...
    test('should display project data correctly', async () => {
      ...
      const hasData = await page.locator('tr:not(:first-child), .bg-white.border-b.border-gray-100').count() > 0;
      
      if (hasData) {
        // If data exists, verify table/card structure
        const firstDataElement = page.locator('tr:not(:first-child), .bg-white.border-b.border-gray-100').first();
  ...
  test.describe('Search Functionality', () => {
    ...
    test('should search and filter results correctly using known data', async () => {
      const page = sharedPage;
      const searchInput = page.locator('input[placeholder*="search" i], input[type="search"], .MuiInputBase-input').first();
      ...
      if (await searchInput.count() > 0) {
        ...
        const initialCount = await page.locator('tr:not(:first-child), .bg-white.border-b.border-gray-100').count();
        ...
        // Get data from the first visible item to search for
        let searchText = '';
        
        // Try to get address from first table row
        const firstRowAddress = page.locator('tbody tr').first().locator('td').nth(1);
        if (await firstRowAddress.count() > 0) {
          const addressText = await firstRowAddress.textContent();
          if (addressText && addressText.trim() !== 'No address provided') {
            // Extract a searchable part of the address (e.g., street number or first word)
            const addressParts = addressText.trim().split(/[\s,]+/);
            searchText = addressParts[0]; // Use first part (street number or first word)
            console.log(`🔍 Using address search term: "${searchText}"`);
          }
        }
        
        // If no address found, try to get client name
        if (!searchText) {
          const firstRowClient = page.locator('tbody tr').first().locator('td').nth(3);
          if (await firstRowClient.count() > 0) {
            const clientText = await firstRowClient.textContent();
            if (clientText && clientText.trim() !== 'N/A') {
              const clientParts = clientText.trim().split(/\s+/);
              searchText = clientParts[0]; // Use first name
              console.log(`🔍 Using client search term: "${searchText}"`);
            }
          }
        }
        
        // If still no search text, use a known status
        if (!searchText) {
          const firstRowStatus = page.locator('tbody tr').first().locator('td').first();
          if (await firstRowStatus.count() > 0) {
            const statusText = await firstRowStatus.textContent();
            if (statusText && statusText.trim()) {
              searchText = statusText.trim().split(/\s+/)[0]; // Use first word of status
              console.log(`🔍 Using status search term: "${searchText}"`);
            }
          }
        }
        ...
        if (searchText && searchText.length > 2) {
          ...
          await page.waitForTimeout(1500); // Wait for search debounce and results
          
          // Get filtered results count
          const filteredCount = await page.locator('tr:not(:first-child), .bg-white.border-b.border-gray-100').count();
          ...
          if (filteredCount > 0) {
            const firstResult = page.locator('tr:not(:first-child), .bg-white.border-b.border-gray-100').first();
            const resultText = await firstResult.textContent();
            
            // Verify the search term appears in the result (case-insensitive)
            const hasSearchTerm = resultText?.toLowerCase().includes(searchText.toLowerCase());
            expect(hasSearchTerm).toBeTruthy();
            
            console.log(`✅ Search validation passed: "${searchText}" found in results`);
          } else {
            console.log(`ℹ️ No results for "${searchText}" - this is valid if the search term is very specific`);
          }
          
          // Clear search to restore original data
          await searchInput.clear();
          await page.waitForTimeout(1000);
          
          // Verify data is restored
          const restoredCount = await page.locator('tr:not(:first-child), .bg-white.border-b.border-gray-100').count();
          expect(restoredCount).toBe(initialCount);
          
          console.log(`✅ Search cleared successfully, data restored to ${restoredCount} items`);
        } else {
          console.log('ℹ️ Could not extract meaningful search term from data');
        }
      }
    });
  ...
  test.describe('Filter Operations', () => {
    ...
    test('should filter data correctly when options are selected', async () => {
      const page = sharedPage;
      ...
      console.log(`📊 Initial data count for filtering: ${initialCount}`);
      
      // Test status filter with actual filtering validation
      const statusFilter = page.locator('select, .MuiSelect-select').first();
      if (await statusFilter.count() > 0 && await statusFilter.isVisible()) {
        ...
        // Look for actual status options (skip "All" options)
        const allOptions = page.locator('[role="option"], option, .MuiMenuItem-root');
        const optionCount = await allOptions.count();
        
        if (optionCount > 1) {
          // Find a non-"All" option to test with
          ...
          for (let i = 1; i < Math.min(optionCount, 5); i++) {
            ...
            if (optionText && !optionText.toLowerCase().includes('all')) {
              ...
              selectedOptionText = optionText; // Store text before clicking
          ...
          if (selectedOption) {
            ...
            await page.waitForTimeout(1500); // Wait for filtering to complete
            ...
            // If we have filtered results, verify they match the filter
            if (filteredCount > 0) {
              console.log(`✅ Filter applied successfully for: "${selectedOptionText}"`);
            }
            
            // Reset filter to "All" to restore data
            await statusFilter.click();
            await page.waitForTimeout(300);
            const allOption = allOptions.filter({ hasText: /all.*status/i }).first();
            if (await allOption.count() > 0) {
              ...
              const restoredCount = await page.locator('tr:not(:first-child), .bg-white.border-b.border-gray-100').count();
              expect(restoredCount).toBe(initialCount);
              console.log(`✅ Filter reset successfully, data restored to ${restoredCount} items`);
            }
          } else {
            console.log('ℹ️ No non-"All" filter options found');
          }
        } else {
          console.log('ℹ️ Filter dropdown has no options or only "All" option');
        }
      }
    });
  ...
  test.describe('View Mode Switching', () => {
    ...
    test('should support view mode toggle between table and cards', async () => {
      ...
      if (!hasData) {
        console.log('ℹ️ No data available for view toggle testing');
        return;
      }
      
      // Look for view toggle button using Material-UI icons
      ...
      if (await viewToggleButton.count() > 0) {
        ...
        // Get initial state (likely table view)
        const initialRows = await page.locator('tbody tr').count();
        const initialCards = await page.locator('.bg-white.border-b.border-gray-100').count();
        
        console.log(`Initial state: rows=${initialRows}, cards=${initialCards}`);
        ...
        const afterCardsRows = await page.locator('tbody tr').count();
        const afterCardsCards = await page.locator('.bg-white.border-b.border-gray-100').count();
        
        console.log(`After toggle: rows=${afterCardsRows}, cards=${afterCardsCards}`);
        
        // Verify view actually changed
        const viewChanged = (afterCardsRows !== initialRows) || (afterCardsCards !== initialCards);
        expect(viewChanged).toBeTruthy();
        
        console.log('✅ View toggle functionality validated');
      } else {
        console.log('ℹ️ View toggle not found - might be mobile view or feature disabled');
      }
    });
  ...
  test.describe('Progressive Card Features', () => {
    ...
    test('should handle progressive disclosure in card view', async () => {
      ...
      const cards = page.locator('.bg-white.border-b.border-gray-100');
  ...
  test.describe('Individual Action Workflows', () => {
    ...
    test('should handle Open action', async () => {
      ...
      const firstProject = page.locator('tbody tr, .bg-white.border-b.border-gray-100').first();
  ...
  test.describe('Data Refresh Functionality', () => {
    ...
    test('should refresh data when refresh button clicked', async () => {
      ...
      if (await refreshButton.count() > 0) {
        // Get initial data count
        const initialCount = await page.locator('tbody tr, .bg-white.border-b.border-gray-100').count();
        ...
        const newCount = await page.locator('tbody tr, .bg-white.border-b.border-gray-100').count();
  ...
  test.describe('Progressive Card States (Phase 2)', () => {
    ...
    test('should support 3-state progressive disclosure', async () => {
      ...
      // Find a card
      const firstCard = page.locator('.bg-white.border-b.border-gray-100, .bg-white.border-b').first();
    ...
    test('should handle card density toggle', async () => {
      ...
      if (await densityButton.count() > 0) {
        const initialCards = page.locator('.bg-white.border-b.border-gray-100, .bg-white.border-b');
  ...
  test.describe('Advanced Filter Logic (Phase 2)', () => {
    ...
    test('should validate filter combinations', async () => {
      ...
      if (await statusFilter.count() > 0 && await productFilter.count() > 0) {
        ...
        const filteredCount = await page.locator('tbody tr, .bg-white.border-b.border-gray-100').count();
        expect(filteredCount).toBeLessThanOrEqual(initialCount);
      }
    });
  });
  ...
  test.describe('Enhanced Search Features (Phase 2)', () => {
    ...
    test('should search specific fields accurately', async () => {
      ...
      if (await searchInput.count() > 0) {
        ...
        let results = await page.locator('tbody tr, .bg-white.border-b.border-gray-100').count();
        ...
        results = await page.locator('tbody tr, .bg-white.border-b.border-gray-100').count();
    ...
    test('should handle search debouncing', async () => {
      ...
      if (await searchInput.count() > 0) {
        ...
        const results = await page.locator('tbody tr, .bg-white.border-b.border-gray-100').count();
  ...
  test.describe('Performance Testing (Phase 3)', () => {
    ...
    test('should handle large dataset performance', async () => {
      ...
      await page.reload();
      await page.waitForSelector('tbody tr, .bg-white.border-b.border-gray-100', { timeout: 10000 });
  ...
  test.describe('Edge Cases and Error Recovery (Phase 3)', () => {
    ...
    test('should handle network errors gracefully', async () => {
      ...
      if (await refreshButton.count() > 0) {
        ...
        const dataElements = page.locator('tbody tr, .bg-white.border-b.border-gray-100');
    ...
    test('should maintain state during browser resize', async () => {
      ...
      // Get initial state
      const initialData = await page.locator('tbody tr, .bg-white.border-b.border-gray-100').count();
      ...
      for (const viewport of viewports) {
        ...
        const currentData = await page.locator('tbody tr, .bg-white.border-b.border-gray-100').count();
  ...
  test.describe('Integration Testing (Phase 3)', () => {
    ...
    test('should handle permissions correctly', async () => {
      ...
      const projectCount = await page.locator('tbody tr, .bg-white.border-b.border-gray-100').count();
