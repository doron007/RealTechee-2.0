...
test.describe('Admin Dashboard Page', () => {
  ...
  test.describe('Quick Actions', () => {
    ...
    test('should provide navigation shortcuts', async () => {
      ...
      if (linkCount > 0) {
        ...
        // Test meaningful navigation validation - check that links have proper destinations
        let validNavCount = 0;
        ...
        for (let i = 0; i < Math.min(linkCount, 3); i++) {
          ...
          if (await link.isVisible()) {
            ...
            if ((href && href.includes('/admin')) || linkText) {
              validNavCount++;
              console.log(`ℹ️ Valid navigation shortcut: "${linkText?.trim()}" → ${href || 'button action'}`);
            }
          }
        }
        
        // Ensure we have meaningful navigation shortcuts
        expect(validNavCount).toBeGreaterThan(0);
      }
    });
  });
  ...
  test.describe('Data Refresh', () => {
    ...
    test('should show data timestamps', async () => {
      ...
      // Look for real-time indicators (separate selectors)
      const realtimeTextElements = page.locator('text=/live|real.?time|now/i');
      const realtimeClassElements = page.locator('.live, .realtime');
      
      const textCount = await realtimeTextElements.count();
      const classCount = await realtimeClassElements.count();
      
      if (textCount > 0 || classCount > 0) {
        console.log(`ℹ️ Real-time data indicators found (text: ${textCount}, classes: ${classCount})`);
      }
    });
  });
