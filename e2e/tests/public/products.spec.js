    ...
    test.describe(`${name} Product Page (${path})`, () => {
      ...
      test.describe('Page Loading & Content Display', () => {
        ...
        test('should load product page without errors', async () => {
          ...
          const mainContent = page.locator('h1, h2, main, .main-content').first();
          await expect(mainContent).toBeVisible();
          
          // Check for meaningful page title
          const pageTitle = await page.title();
          expect(pageTitle).toBeTruthy();
          expect(pageTitle.length).toBeGreaterThan(5);
          console.log(`ℹ️ Page title: "${pageTitle}"`);
          
          // Verify page has product-relevant content (check for product-related text in page)
          const pageContent = await page.textContent('body');
          const hasProductContent = pageContent && (
            pageContent.toLowerCase().includes(name.toLowerCase().split(' ')[0]) ||
          ...
          if (hasProductContent) {
            console.log(`ℹ️ Product-relevant content found for ${name}`);
          }
      ...
      test.describe('Performance & Technical Quality', () => {
        ...
        test('should load within acceptable time', async () => {
          ...
          // Product pages should load reasonably fast (allowing for complex pages)
        ...
        test('should be mobile responsive', async () => {
          ...
          for (const viewport of viewports) {
            ...
            // Check that navigation or CTAs are accessible (may be in mobile menu)
            const visibleCTAs = page.locator('button:visible, a:visible').filter({ hasText: /get|contact|request|menu|home/i });
            const ctaCount = await visibleCTAs.count();
            
            // Should have at least some interactive elements visible (navigation, menu, etc.)
        ...
        test('should have proper SEO optimization', async () => {
          ...
          const h1Text = await page.locator('h1, h2').first().textContent();
          
          // Should have optimized title
          expect(pageTitle).toBeTruthy();
          expect(pageTitle.length).toBeGreaterThan(10);
          expect(pageTitle.length).toBeLessThan(80); // Realistic SEO practice (Google shows ~70 chars)
          
          if (metaDescription) {
            expect(metaDescription.length).toBeGreaterThan(120);
            expect(metaDescription.length).toBeLessThan(200); // Realistic SEO practice (Google shows ~155-160)
            console.log(`ℹ️ Meta description found (${metaDescription.length} chars)`);
          }
